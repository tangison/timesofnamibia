...
export default function TenderAnalysisView() {
  ...
  useEffect(() => {
    const fetchTenders = async () => {
      try {
        const fetchedTenders = await getTenders({ limit: 20 });
    ...
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 md:py-10">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 text-ton-red animate-spin" />
          <span className="ml-3 font-mono text-sm text-ton-black/60">Loading tenders...</span>
        </div>
      </div>
    );
  }
        ...
        <div className="mb-8 sm:mb-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
            ...
            <div className="pt-5 border-t border-ton-black/10">
              ...
              <ul className="space-y-2">
                {selectedTenderData.keyDates.map((d) => (
                  <li key={d.id} className="font-mono text-xs text-ton-black/60 flex items-start gap-2">
          ...
          <div className="mt-6 pt-5 border-t border-ton-black/10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2.5">
              {selectedTenderData.compliances.map((c) => (
                <div key={c.id} className="flex items-start gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 mt-0.5 flex-shrink-0" />
                  <span className="font-sans text-xs text-ton-black/60">{c.requirement}</span>
                </div>
              ))}
            </div>
          </div>
